...
```{r Linear regression - ABs (Spike) vs Metabolites}
...
sub.v2.log <- sub.v2 |> 
  column_to_rownames('sampleid') |> 
  mutate(across(8:15, ~log1p(.)))

design <- model.matrix(~ ds, data = sub.v2.log)

fit <- lmFit(assay(sub.metab.v2), design=design)
fit <- eBayes(fit)

out <- topTable(fit, coef = 2, sort.by = 'p', n=Inf)
...
up.out <- out %>% filter(P.Value < 0.05, logFC > 0)
down.out <- out %>% filter(P.Value < 0.05, logFC < 0)

ora.metab.pos <- enricher(rownames(up.out),
                          TERM2GENE = term2gene, 
                          pvalueCutoff =  Inf, qvalueCutoff = Inf,
                          universe = rownames(assay(sub.metab.v2)))

ora.metab.down <- enricher(rownames(down.out),
                          TERM2GENE = term2gene, 
                          pvalueCutoff =  Inf, qvalueCutoff = Inf,
                          universe = rownames(assay(sub.metab.v2)))

## Correlation
cor.ab.metab <- t(assay(sub.metab.v2)) |> 
  as.data.frame() |> 
  rownames_to_column('sampleid') |> 
...
```{r Linear regression - ABs (Spike) vs Cytokines}
sub.v2 <- read_tsv('results/antibodies/selected_patients_visit2_40.tsv')

cytokines.raw <- ds.mae[['MSD.plasma']] 

sub.cytok <- cytokines.raw[,sub.v2$sampleid]
 
sub.v2.log <- sub.v2 |> 
  column_to_rownames('sampleid') |> 
  mutate(across(8:15, ~log1p(.)))
  
design <- model.matrix(~`SARS-CoV2 Spike`, data = sub.v2.log)

fit <- lmFit(log1p(sub.cytok), design = design)
fit <- eBayes(fit)

summary(decideTests(fit))

out.cytok <- topTable(fit, coef = 2, sort.by = 'p', n=Inf)

## Correlation
cor.ab.cytok <- t(sub.cytok) |> 
  as.data.frame() |> 
  rownames_to_column('sampleid') |> 
  left_join(sub.v2 |> select(sampleid, `SARS-CoV2 Spike`, `SARS-CoV2 S1-RBD`, 
                             `SARS-CoV2 Nucleocapsid`)) |> 
  rstatix::cor_test(vars = contains('SARS'), vars2 = 2:44, method = 'spearman')
