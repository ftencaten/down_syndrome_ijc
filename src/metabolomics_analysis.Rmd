...
ds.mae <- readRDS('data/processed_multiassay_experiment/old/ds_ijc_multiassay_20230622.rds')
...
pvca <- PVCA(metab.imputed.matrix, meta = metadata, threshold = 0.1, inter = F)
...
gsea.metab <- GSEA(rank.metab, TERM2GENE = term2gene, pvalueCutoff = 0.1)  
...
up.out <- cor.ab.metab %>% filter(p < 0.05, cor > 0)
down.out <- cor.ab.metab %>% filter(p < 0.05, cor < 0)
